...
def all(file, snid, name, texps={"g":60, "r":60, "i":60, "z":60}, sortby="dec"):
    
    df = pd.read_csv(file)
    df.drop_duplicates(subset="expnum", inplace=True)
    ...
    for gid, gdf in df.groupby("ifilter"):
        # Sort by specified key
        gdf.sort_values(sortby, inplace=True)

        # Get ra, dec
        ra = np.array([Angle(x, unit=u.deg).deg for x in gdf["ra"]])
        dec = np.array([Angle(x, unit=u.deg).deg for x in gdf["dec"]])

        # Get filter, exptime, fields
        filt = [gid[0]] * len(ra)
        texp = [texps[gid[0]]] * len(ra)
        field = [""] * len(ra)
...
def tiles(json_outpath,json_prefixs,pointRAs,pointDECs,filts,exps,propids,objects,exptypes,programs,fields):
    ...
    outpath = pa.join(cwd, f"GW-MMADS_{tobject}_{date}_{filt[0]}{exps[0]}.json")

    np.savetxt(outpath, json_out, fmt="%s")
